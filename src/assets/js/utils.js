...
async function accountSelect(data) {
  ...
  // Là, tu dispatch l'événement pour prévenir les autres parties de l'appli
  window.dispatchEvent(new CustomEvent("accountChanged", { detail: data }));
}
...
async function setStatus(opt) {
  ...
  if (!statusServer.error) {
    ...
    statusServerElement.innerHTML = `En ligne - ${
      statusServer.ms ? statusServer.ms : 0
    } ms`;
    playersOnline.innerHTML = statusServer.playersConnect
